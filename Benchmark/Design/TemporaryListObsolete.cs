...
public ref struct TemporaryListObsolete<TObject>
    where TObject : class
{
    ...
    {
        private readonly TemporaryListObsolete<TObject> queue;
        private int index;
        private TObject? current;

        public Enumerator(TemporaryListObsolete<TObject> queue)
        {
            this.queue = queue;
            this.index = -1;
